...
class SessionModel {
    ...
    init(config: ConfigModel) async throws {
        ...
        self.speechService = try await SpeechService(artifacts: self.artifacts, experiment: experiment, config: DeepgramConfig(), anonymizationPolicy: .pitchShift(semitones: Float.random(in: -3 ... -1), deleteOriginal: false))
